...
contract KimchiChef is Ownable {
    ...
    function pendingKimchi(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 kimchiReward = multiplier
                .mul(kimchiPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accKimchiPerShare = accKimchiPerShare.add(
                kimchiReward.mul(1e12).div(lpSupply)
            );
        }
        return
            user.amount.mul(accKimchiPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accKimchiPerShare = pool.accKimchiPerShare.add(
            kimchiReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accKimchiPerShare).div(1e12).sub(
            user.rewardDebt
        );
        safeKimchiTransfer(msg.sender, pending);
